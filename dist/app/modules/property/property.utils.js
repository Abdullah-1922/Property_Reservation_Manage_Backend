...
exports.sortReservationsByDates = sortReservationsByDates;
exports.sortReservationsByCreatedBy = sortReservationsByCreatedBy;
function sortReservationsByDates(reservations, targetRoomId, query) {
    ...
    const filteredReservations = reservations.filter(reservation => reservation.rooms.some(room => {
...
//sort by created At
function sortReservationsByCreatedBy(reservations, targetRoomId) {
    // Filter the reservations to only include those with the target room_id
    const filteredReservations = reservations.filter(reservation => reservation.rooms.some(room => room.id_zak_room === targetRoomId));
    return filteredReservations.sort((a, b) => {
